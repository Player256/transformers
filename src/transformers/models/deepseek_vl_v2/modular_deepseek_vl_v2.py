...
class DeepseekVLV2ForCausalLM(DeepseekVLForConditionalGeneration):
    def __init__(self, config: DeepseekVLV2Config):
        ...
        projector_config = config.projector_config if hasattr(config, "projector_config") else None
...
class DeepseekVLV2Processor(DeepseekVLProcessor, ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        enc = self.tokenizer(prompt, return_tensors=return_tensors)

        image_token_id = self.tokenizer.convert_tokens_to_ids(self.image_token)
        images_seq_mask = enc["input_ids"] == image_token_id
